...
class RequestWorker(QThread):
    ...
    def run(self):
        for app in self.followed_apps:
            try:
                response = requests.get(f'{app.replace("https://github.com/", "https://api.github.com/repos/")}/releases', headers = {'Authorization': f'token {self.token["github"]}'} if self.token else None)
